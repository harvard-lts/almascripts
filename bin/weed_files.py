...
def main():
	...
	for configSet in configTable:
		...
		try:
			...
			searchDir = False
		try:
			searchDirs = configSet['search_dirs'].split(',')
		except:
			searchDirs = []
		try:
		...
		# To support the old searchDir for a single directory and 
		# the new searchDirs to search multiple directories
		if searchDir:
			searchDirs.append(searchDir)

		# Check that search directories are supported
		if len(searchDirs) > 0:
			for searchDir in searchDirs:
				if searchDir in dirsNotAllowed:
					...
					searchDirs.remove(searchDir)

			if len(searchDirs) == 0:
				notifyJM.log('fail', f'No supported search Directories were not set for {confProfile}', verbose)
				badConfigSet = True

		else:
			notifyJM.log('fail', f'No search Directories were not set in {confFile} for {confProfile}', verbose)
			badConfigSet = True
				...
				# Handle the weeding of files
					...
					for file in filesSorted:
						if reSearchPattern.match(file):
							...
							elif daysToKeep and not fileWasCompress:
